...
class UserController
{
    ...
    // Función para obtener los médicos disponibles en base a la fecha y especialidad
    public function obtenerMedicosDisponibles($especialidadId, $fecha)
    {
        $query = "SELECT c.id, c.nombre, c.apellido, h.horario 
                  FROM colaboradores c
                  JOIN horarios h ON c.id = h.medico_id
                  WHERE c.especialidad_id = :especialidadId AND h.fecha = :fecha";
        $stmt = $this->db->prepare($query);
        $stmt->bindParam(':especialidadId', $especialidadId, PDO::PARAM_INT);
        ...
        foreach ($medicos as $medico) {
            $medicosDisponibles[$medico['id']]['nombre'] = $medico['nombre'];
            $medicosDisponibles[$medico['id']]['apellido'] = $medico['apellido'];
            $medicosDisponibles[$medico['id']]['horarios'][] = $medico['horario'];
            $horariosDisponibles[] = $medico['horario'];
        }

        return [
            'medicos' => array_values($medicosDisponibles),
            'horarios' => array_unique($horariosDisponibles)
        ];
    }
